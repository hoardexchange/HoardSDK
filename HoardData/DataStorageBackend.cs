...
namespace Hoard
{
    abstract public class DataStorageBackend
    {
        ...
        abstract public DataResult LoadHashFromServer(ulong assetId, out string hash);

        abstract public DataResult LoadDataFromServer(ulong assetId, out byte[] data);

        abstract public DataResult UploadDataToServer(ulong assetId, byte[] data);
    }
    ...
    public class DataStorageBackendLocal : DataStorageBackend
    {
        ...
        public override DataResult LoadHashFromServer(ulong assetId, out string hash)
        {
            byte[] data = null;
            DataResult result = LoadDataFromServer(assetId, out data);
            if (!result.Success)
            {
                hash = null;
                return new DataResult("No data for given assetId");
            }
        ...
        public override DataResult LoadDataFromServer(ulong assetId, out byte[] data)
        {
            data = DataStorageUtils.LoadFromDisk(StorageBasePath + "/" + GameID.ID + "/" + assetId);
            return new DataResult();
        }

        public override DataResult UploadDataToServer(ulong assetId, byte[] data)
        {
    ...
    public class DataStorageBackendHoard : DataStorageBackend
    {
        ...
        public override DataResult LoadHashFromServer(ulong assetId, out string hash)
        {
            ...
            if (task_result.StatusCode != HttpStatusCode.OK)
                return new DataResult(task_result.StatusDescription);
            ...
            hash = null;
            return new DataResult("Error retrieving asset hash");
        }
        ...
        public override DataResult UploadDataToServer(ulong assetId, byte[] data)
        {
            ...
            if (create_task_response.StatusCode != HttpStatusCode.Created)
                return new DataResult(create_task_response.StatusDescription);
            else
                return new DataResult();
        }
